...
function BreakdownRow({ label, value, highlight, muted }: BreakdownRowProps) {
  return (
    <div
      className={`flex justify-between py-2 px-3 rounded ${highlight ? "bg-primary font-semibold text-gray-100 opacity-75" : "text-gray-400"} ${muted ? "text-gray-400" : ""}`}
    >
      <span>{label}</span>
...
function Section({ title, children }: SectionProps) {
  return (
    <div className="mb-5">
      <h3 className="text-xs font-semibold uppercase tracking-wider text-gray-100 mb-1 px-1">
        {title}
      </h3>
      ...
      <div className="flex flex-row gap-5 w-full justify-center">
        ...
        <div className="w-1/3 bg-secondary border border-secondary-400 rounded-xl p-5 shadow-sm">
          <Section title="Salary Summary">
            <BreakdownRow
              label="Basic Salary"
              value={result?.AnnualSalary ? result.AnnualSalary / 12 : 0}
            />
            <BreakdownRow
              label="Non-taxable Allowances"
              value={result?.UntaxableIncome ?? 0}
              muted={(result?.UntaxableIncome ?? 0) === 0}
            />
            <BreakdownRow
              label="Gross Salary"
              value={result?.GrossSalary ?? 0}
            />
            <BreakdownRow
              label="Net Salary (Take-home)"
              value={result?.NetSalary ?? 0}
              highlight
            />
          </Section>
          ...
          <Section title="Deductions (Employee)">
            <BreakdownRow
              label="PhilHealth"
              value={result?.PhilhealthEmployeeContribution ?? 0}
            />
            <BreakdownRow
              label="SSS"
              value={result?.SssEmployeeContribution ?? 0}
            />
            <BreakdownRow
              label="Pag-IBIG"
              value={result?.PagibigContribution ?? 0}
            />
            <BreakdownRow
              label="Withholding Tax (BIR)"
              value={result?.BirContribution ?? 0}
            />
            <BreakdownRow
              label="Total Deductions"
              value={result?.TotalDeductions ?? 0}
              highlight
            />
          </Section>
          ...
          <Section title="Employer Contributions">
            <BreakdownRow
              label="PhilHealth"
              value={result?.PhilhealthEmployerContribution ?? 0}
            />
            <BreakdownRow
              label="SSS"
              value={result?.SssEmployerContribution ?? 0}
            />
            <BreakdownRow
              label="Total Employer Contributions"
              value={result?.EmployerContributions ?? 0}
              highlight
            />
          </Section>
          ...
          <Section title="Annual Projections">
            <BreakdownRow
              ...
              value={result?.AnnualGrossSalary ?? 0}
            />
            <BreakdownRow
              label="Annual Net Salary"
              value={result?.AnnualNetSalary ?? 0}
              highlight
            />
          </Section>
        </div>
