 ...
 */
export function DeductionsCard({ result: r, isSelfEmployed, effectiveSchemeSwitched }: DeductionsCardProps) {
    return (
        <Card className="flex-1 min-w-55 bg-card border-white/5">
            <CardHeader className="px-6 pt-6 pb-0">
                ...
                {isSelfEmployed && r && !effectiveSchemeSwitched && r.TaxScheme === 'flat8' && (
                    <p className="text-[0.65rem] text-white/30 mt-1">8% flat rate applied</p>
                )}
            </CardHeader>
            <CardContent className="px-6 pb-6 pt-3">
                ...
                <DetailRow
                    label={isSelfEmployed
                        ? (r?.TaxScheme === 'flat8' ? 'Income Tax (8% Flat)' : 'Income Tax (Graduated)')
                        : 'Withholding Tax (BIR)'}
