...
// ── Detail row ────────────────────────────────────────────────────────────────
function DetailRow({ label, value, total }: { label: string; value: number; total?: boolean }) {
    return (
...
function NumericField({ label, sublabel, rawValue, onRawChange, onKeyDown }: NumericFieldProps) {
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
        ...
        const allowed = /[\d.]|Backspace|Delete|ArrowLeft|ArrowRight|ArrowUp|ArrowDown|Tab|Home|End/;
    ...
    return (
        <div className="flex-1 min-w-50">
            ...
            <div className={cn(
                ...
                <span className="px-3 text-dash-green font-bold text-base select-none shrink-0">₱</span>
                <Input
                    inputMode="decimal"
                    placeholder="0.00"
                    value={displayed}
...
export function TaxDashboard() {
    ...
    const pieData = [
        { ...PIE_SLICES[0], value: (r?.NetSalary ?? 0) - (r?.UntaxableIncome ?? 0) },
                            ...
                            <CardContent className="px-4 pb-4 flex-1 flex items-center justify-center">
                                {hasData ? (
                                            ...
                                            <Legend
                                                content={() => (
                                                    <ul style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '8px 16px', margin: 0, padding: 0, listStyle: 'none' }}>
                                                        {PIE_SLICES.filter(s => pieData.some(d => d.name === s.name)).map(s => (
                                    ...
                                    <div className="flex flex-col items-center gap-2 py-10 text-center">
                                        ...
                                        <p className="text-white/25 text-sm">Enter a salary and calculate to see the breakdown</p>
                                    </div>
                                )}
                            </CardContent>
                        </Card>
                    ...
                    <div className="flex gap-4 flex-wrap">
                        ...
                        <Card className="flex-1 min-w-55 bg-card border-white/5">
                            ...
                            <CardContent className="px-6 pb-6 pt-3">
                                <DetailRow label="PhilHealth" value={r?.PhilhealthEmployeeContribution ?? 0} />
                                <Separator className="bg-white/6" />
                                <DetailRow label="SSS" value={r?.SssEmployeeContribution ?? 0} />
                                <Separator className="bg-white/6" />
                                <DetailRow label="Pag-IBIG" value={r?.PagibigContribution ?? 0} />
                                <Separator className="bg-white/6" />
                                <DetailRow label="Withholding Tax (BIR)" value={r?.BirContribution ?? 0} />
                        ...
                        <Card className="flex-1 min-w-55 bg-card border-white/5">
                            ...
                            <CardContent className="px-6 pb-6 pt-3">
                                <DetailRow label="Annual Gross Salary" value={r?.AnnualGrossSalary ?? 0} />
                                <Separator className="bg-white/6" />
                                <DetailRow label="Annual Basic Salary" value={r?.AnnualSalary ?? 0} />
                                <Separator className="bg-white/15 my-1" />
                                <DetailRow label="Annual Net Salary" value={r?.AnnualNetSalary ?? 0} total />
                            </CardContent>
                        </Card>
